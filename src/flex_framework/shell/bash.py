...
class BashEmulator(SimpleShellProxy):
    def emulate_bash(self):
        bash_command_string = 'bash --init-file <(echo \'' \
                              '. $HOME/.bashrc; ' \
                              'export PATH="' \
                              + self.env["FLEX_SHELL_PROXY_LOCAL_PATH"] + \
                              ':$PATH"; ' \
...
class BashEmulatorFlexAware(BashEmulator):
    ...
    def __init__(
        ...
        super(BashEmulatorFlexAware, self).__init__(environment=environment, env_path_to_remove=None)
    ...
    def init_env_variables(self):
        ...
        self.env["FLEX_SHELL_PROXY_LOCAL_PATH"] = ":".join(self.get_local_path_entries())
        self.env["PATH"] = self.env["FLEX_SHELL_PROXY_LOCAL_PATH"] + ":" + self.env["PATH"]
    ...
    def read_environment_variables(self, file: str):
        full_env_vars = subprocess.check_output(
            'env -i bash --noprofile --norc -c "set -o allexport; source ' + file + '; set +o allexport; printenv"',
        ...
        for var_line in full_env_vars.split("\n"):
            key_pair = var_line.split("=", 1)
            if len(key_pair) == 2:
                if key_pair[0] not in original_env_vars_dict or original_env_vars_dict[key_pair[0]] != key_pair[1]:
