...
class LcliPrompt(Cmd, AppModeBase):
    ...
    def __init__(self, app: App, command_prefix='', completekey='tab', stdin=None, stdout=None):
        ...
        self.space_separator = self._app.get_config_object().get('app_space_separator', "\n\n\n\n")
        self.app_help_message = self._app.get_config_object().get('app_help_message', '')
        self.intro = self.space_separator + \
                     figlet.renderText(str(cli_code).upper()) + \
                     self.space_separator + \
                     self._app.__.red(
                         self._app.get_config_object().get('app_welcome_message', "Welcome! Type ? to list commands")
                     )
        self.doc_header = self._app.get_config_object().get('app_doc_header', self.doc_header)
        self.refresh_prompt()
    ...
    def refresh_prompt(self):
        ...
        if self.current_command not in ignored_cli_info:
            info_parts = self._invoke_command(self.current_command, 'cli_info', 'list', default=[])
    ...
    def _invoke_command(self, command_path: str, command: str, output: str = 'implicit', default=None):
        try:
            command_object = self._app.get_commands().get_command_by_path(path=command_path.split(),
                                                                          return_first_executable=True)
            if command_object is None:
                raise AppModeException('Command not be found in config: ' + command_path + ' ' + command)

            if 'args' in command_object and \
                    command_object.args.config.get('verbose', True) and \
                    command not in ['h', 'cli_info']:
    ...
    def _invoke_by_command_object(self, command: Command, method: str, default=None):
        ...
        if hasattr(command_object, method):
            method = getattr(command_object, method)
            result = method()
            return result
    ...
    def _invoke_shell(self, command: str, output: str = 'implicit'):
        try:
            ...
            command_runner = self._create_shell_command_runner_object(full_command, output)
            return command_runner()
        except Exception as e:
            self._app.logger.warning(e)
            return 0

    def _create_shell_command_runner_object(self, command: str, output: str = 'implicit') -> CommandRunner:
        command_data = {
            ...
            Command.Constants.ARGS: {
                ...
                CommandArgs.Constants.COMMAND_CONFIG: {
                    'output': output
                }
            }
        }
        return CommandRunner(
            ...
            {},
            command_input_reader_factory=self._app.get_object_manager().provide(ParametersReader.Factory)
        )
