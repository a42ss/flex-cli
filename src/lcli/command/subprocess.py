...
from lcli.config import Command as ConfigCommand, Config
...
class CommandRunner:
    ...
    def __call__(self, **args: str):
        try:
            command_input_reader = self._command_input_reader_factory.create(self._command.args.params,
                                                                             self._local_variables)
            answers = command_input_reader.read()
            command = ''
            if self._parent_command is not None and self._parent_command.args is not None:
            ...
            if output_type == 'list':
                process = subprocess.Popen(command, shell=True, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
                out, err = process.communicate()
                if process.returncode != 0:
                    return []

                processed_out = str(out.rstrip().decode())
                list_parts = [y for y in (x.strip() for x in processed_out.splitlines()) if y]
                return list_parts

            if output_type == 'json':
                p = subprocess.Popen(command, shell=True, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
            ...
            for param in params:
                ...
                if param_name != '':
                    if param_name[1:] in args:
                        ...
                        if param_object.output_format is not None and len(param_object.output_format):
                            value_template = Template(param_object.output_format)
                            command = command + value_template.substitute({"value": param_value, "name": param_name})
                        else:
                ...
                if param_object.output_format is not None and len(param_object.output_format):
                    if param in answers:
                        value_template = Template(param_object.output_format)
                        answer_output = value_template.substitute({"value": answers[param], "name": param_name})
                else:
                    if param in answers:
