...
class ParametersReader(object):
    ...
    class Factory(object):
        @classmethod
        def create(cls, params: CommandArgsParams, defaults: Config) -> 'ParametersReader':
        ...
        if self._params is not None:
            ...
            for param in self._params:
                ...
                if param_object.type == 'config':
                    try:
                        answers[param] = self._defaults.get(param_object.get('config_path'))
                ...
                if param_object.type == 'list' or param_object.type == 'autocomplete':
                    ...
                    question['name'] = param
                    if param_object.choices_cmd is not None and not param_object.choices_cmd.is_empty():
                        choices_cmd = param_object.choices_cmd
                        stdout = subprocess.check_output(choices_cmd.command.split())
                        out = stdout.decode()
                        choices_result = [b.strip(choices_cmd.filter['strip']) for b in out.splitlines()]
