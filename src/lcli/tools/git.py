...
def _shell_execute(command: str, output: bool = False):
    if output:
        return subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
    else:
...
class Git(BaseTool):
    ...
    def __init__(self, app: App) -> None:
        ...
        self._command_variables = self._app.get_config_object().get(
            ['commands_defaults', self._command_name],
            default={}
        )
    ...
    def save(self, message: str = ''):
        ...
        if message != '':
            ...
            self.commit(self.branch() + ' ' + self._command_variables.get('autocommit_message'))
        self.push()
