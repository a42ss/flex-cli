...
from flex_framework.api.application import ApplicationInterface, ApplicationResultInterface
from flex_framework.api.exception import FlexExceptionInterface
from flex_framework.console.handler import HandlerRouterFactory, HandlerRouter
...
class FlexCli(ApplicationInterface[CliResponse]):
    ...
    def launch(self) -> ApplicationResultInterface:
        try:
            handler_router: HandlerRouter = self._handler_router_factory.create(HandlerRouter)
            handler = handler_router.find(handler_router.create_handler_path(self._input.handler))
            result: ApplicationResultInterface = handler.handle()
            return result
        except FlexExceptionInterface as exception:
            return CliResponse.Factory.create(exception.get_code(), exception.get_message())
